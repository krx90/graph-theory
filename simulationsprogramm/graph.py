...
class GraphMitDatenUndRundreisen(GraphMitDaten):
    def __init__(self):
        GraphMitDaten.__init__(self)
        self.route = None
        self.routeOhneStartKnoten = None

    def naechste_permutation(self, L):
    ...
    def laenge(self, rundreise):
        weglaenge = 0
        anzahlKnoten = len(rundreise)
        i = 1
        while i < anzahlKnoten:
            weglaenge = weglaenge + float(self.getGewichtKante(rundreise[i-1], rundreise[i]))
            i = i+1
        return weglaenge
       
    def minRundreise(self, startKnoten):
        startRouteOhneStartKnoten = []
        for knoten in self.getAlleKnoten():
        ...
        while not endePermutationen:
            routeOhneStartKnoten = self.naechste_permutation(routeOhneStartKnoten)
    ...
    def ersteRundreise(self, startKnoten):
        self.routeOhneStartKnoten = []
        for knoten in self.getAlleKnoten():
            if knoten != startKnoten:
                self.routeOhneStartKnoten = self.routeOhneStartKnoten + [knoten]
        self.route = [startKnoten] + self.routeOhneStartKnoten + [startKnoten]

    def naechsteRundreise(self, startKnoten):
        self.routeOhneStartKnoten = self.naechste_permutation(self.routeOhneStartKnoten)
        self.route = [startKnoten] + self.routeOhneStartKnoten + [startKnoten]
