...
class GraphDijkstra(GraphMitDaten):
    def __init__(self):
        GraphMitDaten.__init__(self)

    def kuerzesterWegDijkstra(self, startKnoten, zielKnoten):
        if self.existiertKnoten(startKnoten) and self.existiertKnoten(zielKnoten):
            refStartKnoten = self.getRefKnoten(startKnoten)
            refZielKnoten = self.getRefKnoten(zielKnoten)
            # Vorbereitung
            for refKnoten in self.knotenListe:
                if refKnoten != refStartKnoten:
                    refKnoten.abstand = 'u'
                    refKnoten.herkunft = None
                else:
                    refKnoten.abstand = 0
                    refKnoten.herkunft = None
            zuVerarbeiten = [refStartKnoten]
            refMinKnoten = None
            # Verarbeitung der Knoten
            while zuVerarbeiten != [] and refMinKnoten != refZielKnoten:
                # bestimme einen Knoten minKnoten aus zuVerarbeiten mit minimalem Abstand
                refMinKnoten = zuVerarbeiten[0]
                for refKnoten in zuVerarbeiten[1:]:
                    if refKnoten.abstand != 'u':
                        if refKnoten.abstand < refMinKnoten.abstand:
                            refMinKnoten = refKnoten
                # entferne minKnoten aus zuVerarbeiten
                zuVerarbeitenNeu = []
                for refKnoten in zuVerarbeiten:
                    if refKnoten != refMinKnoten:
                        zuVerarbeitenNeu = zuVerarbeitenNeu + [refKnoten]
                zuVerarbeiten = zuVerarbeitenNeu
                # bearbeite die Nachbarn von minKnoten
                for refKante in refMinKnoten.kantenZuNachbarn: 
                    refNachbarKnoten = refKante.zielKnoten
                    gewicht = float(self.getKantenRefDaten(refKante, 'gewicht'))
                    neuerAbstand = refMinKnoten.abstand + gewicht
                    if refNachbarKnoten.abstand == 'u':
                        zuVerarbeiten = zuVerarbeiten + [refNachbarKnoten]                
                        refNachbarKnoten.abstand = neuerAbstand
                        refNachbarKnoten.herkunft = refMinKnoten
                    else:
                        if refNachbarKnoten in zuVerarbeiten:
                            if refNachbarKnoten.abstand > neuerAbstand:
                                refNachbarKnoten.abstand = neuerAbstand
                                refNachbarKnoten.herkunft = refMinKnoten
            weglaenge = refZielKnoten.abstand
            # Erzeugung des Weges zum zielKnoten
            weg = []
            refKnoten = refZielKnoten
            while refKnoten != refStartKnoten and refKnoten.herkunft != None:
                weg = [(refKnoten.herkunft.name, refKnoten.name)] + weg
                refKnoten = refKnoten.herkunft
        else:
            weg = []
            weglaenge = 'u'
